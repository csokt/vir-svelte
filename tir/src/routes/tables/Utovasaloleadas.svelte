<script>
  ...
  onMount( async () => {
    $pagetitle = 'Szeged utóvasaló napi leadás'
    const sql = `select top 500 * from monitor_szegedutovasaloleadas order by [Cikk/IT]`
...
<div class="simple-datatables-full-height {tablewidth}">
  <Datatable settings={simple_datatables_settings} data={tabledata}>
    <thead>
      <th data-key="Cikk/IT">Cikk/IT</th>
    ...
    {#each $rows as row}
      <tr>
        <td class="text-blue-800" on:click={e => dispatch('seasearch', row['Cikk/IT'].split('/')[0])}>{row['Cikk/IT']}</td>
